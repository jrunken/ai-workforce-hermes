...
1. **Read the past 7 days of daily logs** (`~/chief-workspace/memory/YYYY-MM-DD.md`)
   - Compile a list of all significant events, decisions, and outcomes

2. **Write a weekly summary** → `~/chief-workspace/memory/weekly/YYYY-WXX.md`
   ```markdown
...
3. **Review trust progression** → `~/chief-workspace/bank/trust.md`
...
4. **Review opinions** → `~/chief-workspace/bank/opinions.md`
...
5. **Check entity staleness** → `~/chief-workspace/bank/entities/*.md`
   - Any entities not referenced in 30+ days? Flag in `~/chief-workspace/bank/index.md` under "Stale Items"
   - Any entities referenced heavily? Make sure their pages are current

6. **Review processes** → `~/chief-workspace/bank/processes.md`
   - Any new SOPs that emerged from repeated tasks this week?
   - Any existing processes that need updating based on new learnings?

7. **Check shared knowledge** → `~/chief-workspace/shared/`
...
8. **Review worker delegation success rates** → `~/chief-workspace/bank/experience.md`
