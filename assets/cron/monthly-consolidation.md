...
1. **Read all weekly summaries from this month** (`~/chief-workspace/memory/weekly/YYYY-WXX.md`)

2. **Write a monthly summary** → `~/chief-workspace/memory/monthly/YYYY-MM.md`
   ```markdown
   # Month: YYYY-MM
   ...
   ## Trust State
   [Current trust levels table, copied from ~/chief-workspace/bank/trust.md]
...
3. **Archive old daily logs**
   - Move daily logs older than 60 days to `~/chief-workspace/memory/archive/YYYY/MM/`
...
4. **Deep prune Hermes memory**
   ...
   - Archived items go to `~/chief-workspace/memory/archive/pruned.md`

5. **Entity maintenance** → `~/chief-workspace/bank/entities/`
   - Archive entities inactive for 90+ days → `~/chief-workspace/bank/archive/`
   - Ensure active entity pages are current and well-structured
   - Clean up any duplicate or redundant entity pages

6. **Process review** → `~/chief-workspace/bank/processes.md`
